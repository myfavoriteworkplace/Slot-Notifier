...
export function getBookingActionState({ booking }: { booking: BookingWithSlot }): BookingActionState {
  ...
  return {
    ...
    canOverrideComplete: !isTerminal && !isCompleted && !isTreatmentCompleted && !isInConsultation && !isCheckedIn,
    canPatientLeftEarly: !isTerminal && !isCompleted && !isTreatmentCompleted && !isInConsultation && !isCheckedIn,
