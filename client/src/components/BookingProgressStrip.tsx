...
export interface BookingProgressStripProps {
  ...
  isCancelled?: boolean;
  isNoShow?: boolean;
  isOverride?: boolean;    // Admin force-closed, skipping intermediate stages → orange strikethrough
  isLeftEarly?: boolean;   // Patient physically walked out before treatment complete → amber trail
  hasUnpaidBill?: boolean; // Visit closed but a bill is still pending payment → amber final dot
  noBill?: boolean;        // Visit closed with no bill created at all → amber final dot
}
...
export function BookingProgressStrip({
  ...
  // ── Terminal failure states: cancelled / no-show / patient left early ──
  if (isCancelled || isNoShow || isLeftEarly) {
    const trailColor   = isLeftEarly ? "bg-amber-400/60"  : "bg-red-400/60";
    const dotDoneBg    = isLeftEarly ? "bg-amber-50 dark:bg-amber-950/20 border-amber-300 dark:border-amber-700"
                                     : "bg-red-100 dark:bg-red-900/30 border-red-300 dark:border-red-700";
    const dotDoneInner = isLeftEarly ? "bg-amber-400" : "bg-red-400";
    const labelDone    = isLeftEarly ? "text-amber-500 dark:text-amber-400"
    ...
    return (
      <div className="flex items-center gap-1.5 px-1 py-0.5 overflow-x-auto no-scrollbar">
        {STAGES.map((s, i) => {
          ...
          return (
            <div key={s.key} className="flex items-center gap-1 shrink-0">
              {i > 0 && (
                <div className={`h-px w-3 sm:w-4 ${wasDone ? trailColor : "bg-border/40"}`} />
              )}
              <div className="flex flex-col items-center gap-0.5">
                ...
                <span className={`text-[9px] font-medium leading-none whitespace-nowrap ${wasDone ? labelDone : "text-muted-foreground/50"}`}>
                  {s.label}
                </span>
              </div>
            </div>
        ...
        <div className={`ml-1.5 shrink-0 flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full ${badgeCls}`}>
  ...
  return (
    <div className="flex items-center gap-1 px-1 py-0.5 overflow-x-auto no-scrollbar">
      {STAGES.map((s, i) => {
        ...
        // Skipped: admin override to stage 4, but this stage was never naturally reached
        ...
        if (isSkipped) {
          ...
          // Final "Visit Done" dot — billing-aware colour
      ...
      {/* Billing state badge — only on completed visits with billing issues */}
      {stage === 4 && (hasUnpaidBill || noBill) && (
        <div className="ml-1 shrink-0 flex items-center gap-1 text-[9px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 px-1.5 py-0.5 rounded-full">
