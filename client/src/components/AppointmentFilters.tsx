...
export function AppointmentFilters({
  ...
  return (
    <div
      ...
      className="animate-in fade-in slide-in-from-top-1 duration-200 rounded-xl border border-border/50 bg-card p-3 sm:p-4 shadow-sm"
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Refine the appointments shown below.</p>
          </div>
          {activeCount > 0 && (
        ...
        <button
          ...
          className="h-11 w-11 shrink-0 rounded-lg text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
          data-testid="button-close-appointment-filters"
        >
      ...
      <div className="space-y-3">
        <section className="rounded-xl border border-border/40 bg-muted/20 p-3" aria-labelledby="filter-date-label">
          <div className="mb-2 flex items-center justify-between gap-2">
            ...
            {(filterDate || filterEndDate) && (
              <button type="button" onClick={clearDates} className="text-xs font-semibold text-muted-foreground hover:text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50" data-testid="button-clear-date-filter">
                Clear
              </button>
            )}
          </div>
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" className="min-h-[44px] justify-start rounded-lg border border-border/60 bg-background text-base font-medium sm:text-sm" aria-label="Choose start date" data-testid="button-filter-start-date">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" disabled={!filterDate} className="min-h-[44px] justify-start rounded-lg border border-border/60 bg-background text-base font-medium sm:text-sm" aria-label="Choose end date" data-testid="button-filter-end-date">
        ...
        <section className="rounded-xl border border-border/40 bg-muted/20 p-3" aria-labelledby="filter-quick-label">
          <div className="mb-2 flex items-center justify-between gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            {[
              { value: "this-week" as const, label: "This Week", count: thisWeekCount },
              { value: "next-week" as const, label: "Next Week", count: nextWeekCount },
            ].map(item => (
              <button key={item.value} type="button" onClick={() => { clearDates(); setQuickFilter(quickFilter === item.value ? "all" : item.value); }} aria-pressed={quickFilter === item.value} className={`min-h-[44px] rounded-full border px-3 text-xs font-semibold transition-all active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${quickFilter === item.value ? "border-primary bg-primary text-primary-foreground" : "border-border/60 bg-background text-muted-foreground hover:border-primary/40 hover:text-foreground"}`} data-testid={`chip-filter-${item.value}`}>
            ...
            {presets.map(preset => {
              ...
              return <button key={preset.value} type="button" onClick={() => selectPreset(preset.from, preset.to)} aria-pressed={active} className={`min-h-[44px] rounded-full border px-3 text-xs font-semibold transition-all active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${active ? "border-primary bg-primary text-primary-foreground" : "border-border/60 bg-background text-muted-foreground hover:border-primary/40 hover:text-foreground"}`} data-testid={`chip-date-${preset.value}`}>{preset.label}</button>;
            })}
          </div>
        </section>

        <section className="rounded-xl border border-border/40 bg-muted/20 p-3" aria-labelledby="filter-status-label">
          <div className="mb-2 flex items-center justify-between gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            {statusOptions.map(({ value, label, Icon, tone }) => {
              ...
              return <button key={value} type="button" onClick={() => setStatusFilter(active ? "" : value)} aria-pressed={active} className={`min-h-[44px] rounded-full border px-3 text-xs font-semibold transition-all active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 ${toneClasses}`} data-testid={`chip-status-${value}`}><Icon className="mr-1 inline h-3.5 w-3.5" aria-hidden="true" />{label}</button>;
            })}
          </div>
        </section>

        <section className="rounded-xl border border-border/40 bg-muted/20 p-3" aria-labelledby="filter-role-label">
          <div className="mb-2 flex items-center justify-between gap-2">
        ...
        {(activeCount > 0 || dateLabel) && (
          <div className="rounded-xl border border-border/40 bg-muted/30 p-3" aria-live="polite">
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Active filters</span>
              <button type="button" onClick={clearAll} className="text-xs font-semibold text-destructive hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50" data-testid="button-clear-all-filters">Clear all</button>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {quickFilter === "this-week" || quickFilter === "next-week" ? <button type="button" onClick={() => setQuickFilter("all")} className="rounded-full border border-border bg-background px-2.5 py-1 text-xs font-medium text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50">{quickFilter === "this-week" ? "This Week" : "Next Week"} <X className="ml-1 inline h-3 w-3" aria-hidden="true" /></button> : null}
        ...
        {activeCount > 0 && <button type="button" onClick={clearAll} className="min-h-[44px] w-full rounded-lg border border-destructive/30 bg-destructive/5 text-xs font-semibold text-destructive transition-colors hover:bg-destructive/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50" data-testid="button-clear-all-filters-bottom">Clear all filters</button>}
      </div>
    </div>
  );
}