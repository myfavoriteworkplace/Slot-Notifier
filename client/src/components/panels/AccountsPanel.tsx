...
import {
  IndianRupee, Download, Filter, Clock, CheckCircle2, ChevronDown, Loader2,
  Trash2,
} from "lucide-react";
...
interface AccountsPanelProps {
  ...
  accountsView: 'ledger' | 'register';
  setAccountsView: (v: 'ledger' | 'register') => void;
  accountsSearch: string;
  setAccountsSearch: (v: string) => void;
  accountsStatusFilter: 'all' | 'paid' | 'pending' | 'partial' | 'overdue';
  setAccountsStatusFilter: (v: 'all' | 'paid' | 'pending' | 'partial' | 'overdue') => void;
  expandedPatients: Set<string>;
  setExpandedPatients: (fn: (prev: Set<string>) => Set<string>) => void;
  updateBillStatusMutation: { mutate: (data: { id: number; paymentStatus: string }) => void; isPending: boolean; variables?: { id: number } | null };
  deleteBillMutation: { mutate: (id: number) => void; isPending: boolean };
  billDeleteConfirm: number | null;
  setBillDeleteConfirm: (id: number | null) => void;
  printBillFromRecord: (bill: PatientBill) => void;
  setActivePanel: (panel: string) => void;
  setSelectedPatientId: (id: number) => void;
  notify: { success: (msg: string, opts?: { description?: string }) => void };
}
...
export default function AccountsPanel({
  ...
  accountsStatusFilter, setAccountsStatusFilter,
  expandedPatients, setExpandedPatients,
  updateBillStatusMutation,
  deleteBillMutation,
  billDeleteConfirm, setBillDeleteConfirm,
  printBillFromRecord,
  setActivePanel,
