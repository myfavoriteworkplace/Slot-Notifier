...
export default function AdminMessagingUsagePanel({
  ...
  const totalClinicsWithUsage = query.data?.clinics.filter(clinic => clinic.total > 0).length;
  ...
  const channelMix: Array<{ label: string; value: number | null | undefined; Icon: LucideIcon; bar: string; text: string }> = [
    { label: "SMS", value: query.data?.totals.sms, Icon: Smartphone, bar: "bg-sky-500", text: "text-sky-700" },
    { label: "WhatsApp", value: query.data?.totals.whatsapp, Icon: MessageSquare, bar: "bg-emerald-500", text: "text-emerald-700" },
    { label: "Email", value: query.data?.totals.email, Icon: Mail, bar: "bg-violet-500", text: "text-violet-700" },
  ];
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <div className="min-w-[850px]">
              ...
              {query.isLoading && <p className="px-4 py-10 text-center text-sm text-muted-foreground">Loading clinic usage…</p>}
              {query.isError && <p className="px-4 py-10 text-center text-sm text-red-600">Clinic usage unavailable.</p>}
              {query.data && !filteredClinics.length && <p className="px-4 py-10 text-center text-sm text-muted-foreground">No clinics match this search.</p>}
            </div>
          </div>
        </CardContent>
      </Card>}
