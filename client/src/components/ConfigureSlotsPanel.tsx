...
import {
  Clock, CalendarIcon, CalendarDays, ChevronLeft, ChevronRight,
  Save, Loader2, X, AlertTriangle, Info, ChevronUp, ChevronDown, Sun,
} from "lucide-react";
import {
  format, addDays, startOfToday, startOfDay, startOfWeek,
  isAfter, isSameDay, differenceInCalendarDays,
} from "date-fns";
import {
  DayConfig, DEFAULT_SLOT_TIMINGS, DEFAULT_SECTION_CAPACITY,
...
interface ConfigureSlotsPanelProps {
  clinic: { id: number } | null | undefined;
...
export default function ConfigureSlotsPanel({ clinic, isAuthenticated }: ConfigureSlotsPanelProps) {
  ...
  useEffect(() => {
    ...
    const newEntries: Record<string, DayConfig> = {};
    for (const slot of savedSlotConfigs) {
      const dt = new Date(slot.startTime);
      const dateStr = format(dt, "yyyy-MM-dd");
      const matchedTiming = DEFAULT_SLOT_TIMINGS.find(
        (st) => st.startHour === dt.getHours() && st.startMinute === dt.getMinutes()
      );
      if (!matchedTiming) continue;
      if (!newEntries[dateStr]) newEntries[dateStr] = { isClosed: dt.getDay() === 0, sections: {} };
      newEntries[dateStr].sections[matchedTiming.id] = {
    ...
    for (const [dateStr, cfg] of Object.entries(newEntries)) {
      const allCancelled =
        Object.values(cfg.sections).length === DEFAULT_SLOT_TIMINGS.length &&
  ...
  const getDefaultSectionsForDate = () =>
    Object.fromEntries(
      slotTimings.map((s) => [
        s.id,
        { maxBookings: DEFAULT_SECTION_CAPACITY[s.id] ?? 3, isCancelled: false },
      ])
    );
  ...
  const getConfigForDate = (date: Date): DayConfig => {
    ...
    return dayConfigCache[dateStr] ?? {
      ...
      sections: getDefaultSectionsForDate(),
    };
  };
  ...
  const updateDayClosedState = (date: Date, isClosed: boolean) => {
    ...
    setDayConfigCache((prev) => {
      const existing = prev[dateStr] ?? { isClosed: date.getDay() === 0, sections: getDefaultSectionsForDate() };
      ...
      return {
        ...
        [dateStr]: {
          ...
          sections: {
            ...existing.sections,
            [slotId]: {
              ...(existing.sections[slotId] ?? { maxBookings: DEFAULT_SECTION_CAPACITY[slotId] ?? 3, isCancelled: false }),
              maxBookings: value,
            },
          },
        },
  ...
  const saveDayConfiguration = async () => {
    ...
    try {
      const datesToSave = rangeStart && rangeEnd ? getDatesInRange(rangeStart, rangeEnd) : [configDate];
            ...
            <div className="flex items-center justify-between gap-3 px-1 py-1">
              <div className="flex items-center gap-2.5 flex-wrap">
                <span className="flex items-center gap-1 text-xs font-medium text-blue-600 dark:text-blue-400">
                  <span className="h-2.5 w-2.5 rounded-sm bg-blue-500/30 border border-blue-400/60 inline-block" />Selected
                </span>
                <span className="flex items-center gap-1 text-xs font-medium text-rose-500">
                  <span className="h-2.5 w-2.5 rounded-sm bg-rose-500/20 border border-rose-400/40 inline-block" />Closed
                </span>
                ...
                <span className="flex items-center gap-1 text-xs font-medium text-muted-foreground/50">
                  <span className="h-2.5 w-2.5 rounded-sm bg-muted/60 border border-border/30 inline-block" />Past — locked
                </span>
              </div>
              ...
              return (
                <div className="w-full overflow-x-auto rounded-xl border border-border/40">
                  <div className="min-w-[580px]">
                        ...
                        return (
                          <button
                            ...
                            className={`relative px-1 py-2.5 text-center border-l border-border/40 transition-all ${
                              isPast ? "opacity-40 cursor-not-allowed bg-muted/40"
                              : isEdge ? "bg-blue-500/30 ring-1 ring-inset ring-blue-400/60"
                          ...
                          >
                            ...
                            <div className={`text-base font-black mt-0.5 leading-none ${
                              ...
                              : isToday ? "h-6 w-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center mx-auto text-[13px] font-bold"
                    ...
                    {slotTimings.map((slot) => (
                      <div key={slot.id} className="grid border-b border-border/20 last:border-0" style={{ gridTemplateColumns: "100px repeat(7, 1fr)" }}>
                          ...
                          return (
                            <button
                              ...
                              className={`px-1 py-2 border-l border-border/20 flex flex-col items-center justify-center min-h-[44px] transition-all ${
                                isPast ? "opacity-35 cursor-not-allowed bg-muted/20"
                                : isSelected ? "bg-blue-500/15 active:bg-blue-500/25"
                                : isToday ? "bg-primary/5 active:bg-primary/10"
                                : "hover:bg-muted/25 active:bg-muted/40"
                              }`}
                            >
                              {isClosed ? (
                                <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full leading-none ${
                                  isPast ? "text-muted-foreground/60 bg-muted border border-border/30"
            ...
            {(() => {
              ...
              return (
                <div className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                  cbCfg.isClosed ? "bg-rose-50 dark:bg-rose-500/10 border-rose-200 dark:border-rose-500/30" : "bg-muted/20 border-border/40"
                }`}>
                  <Switch
                    ...
                    <div className="p-4 space-y-3">
                                  ...
                                  <div className="flex items-center shrink-0">
                                    <Input
                                      ...
                                      onChange={(e) => {
                                        ...
                                        getActiveDates().forEach((d) => updateSectionCapacity(d, slot.id, v));
                                      }}
                      ...
                      <div className="border-t border-border/30 pt-3 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          >
                            {isBulkApplying ? <Loader2 className="h-4 w-4 text-primary animate-spin" /> : <CalendarDays className="h-4 w-4 text-primary" />}
                          ...
                          >
                            {isBulkApplying ? <Loader2 className="h-4 w-4 text-amber-500 animate-spin" /> : <Sun className="h-4 w-4 text-amber-500" />}
                      ...
                      <div className="rounded-xl border border-border/40 overflow-hidden">
                        ...
                        >
                          ...
                          {showHowItWorks ? <ChevronUp className="h-3.5 w-3.5 text-muted-foreground" /> : <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />}
                        </button>
                        {showHowItWorks && (
                              ...
                              <div className="px-5 py-4 space-y-3">
                                ...
                                <div className="rounded-xl border border-border/40 bg-muted/20 p-3 space-y-2">
                                  ...
                                  <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
                                    {rangeDays.map((d) => (
                                      <span key={d.toISOString()} className={`text-xs font-medium border px-2 py-0.5 rounded-full ${d.getDay() === 0 || d.getDay() === 6 ? "bg-rose-50 dark:bg-rose-500/10 border-rose-200 dark:border-rose-500/25 text-rose-600 dark:text-rose-400" : "bg-background border-border/60"}`}>
                                        {format(d, "EEE d MMM")}
                                      </span>
                                    ))}
                                  </div>
