  ...
  return <div className="max-w-6xl space-y-4">
    <div className="flex items-start justify-between gap-3">
      <div>
        ...
        <p className="text-sm text-muted-foreground">See how much storage your clinic uses, review files that need attention, and understand upload limits.</p>
      </div>
    ...
    <div className="grid items-start gap-4 lg:grid-cols-[1.3fr_1fr]">
      <Card className="overflow-hidden">
        ...
        <CardContent className="space-y-4 p-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between text-[11px] text-muted-foreground"><span>Files using storage</span><span>{formatBytes(data?.quota.remainingBytes ?? 0)} available</span></div>
          </div>
          <div className="grid grid-cols-2 gap-2 border-t pt-3">
            <div className="rounded-md bg-slate-50 p-2.5 dark:bg-slate-900/40"><p className="text-[11px] text-muted-foreground">Saved files</p><p className="text-sm font-bold">{data?.tracked.files ?? 0}</p></div>
            <div className="rounded-md bg-slate-50 p-2.5 dark:bg-slate-900/40"><p className="text-[11px] text-muted-foreground">Storage available</p><p className="text-sm font-bold">{formatBytes(data?.quota.remainingBytes ?? 0)}</p></div>
          </div>
          <div className="flex items-center justify-between gap-3 border-t pt-3">
            <p className="text-xs text-muted-foreground">Need more storage?</p>
            <Button size="sm" className="h-8 bg-emerald-600 text-xs text-white hover:bg-emerald-700" onClick={() => setLocation("/pricing")}>View plans</Button>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-sm uppercase tracking-wide"><ShieldCheck className="h-4 w-4 text-emerald-600" />File storage checks</CardTitle></CardHeader>
          <CardContent className="divide-y p-0">
            <div className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="min-w-0"><p className="text-xs font-semibold">Files stored</p><p className="truncate text-[11px] text-muted-foreground">{data?.exact ? `${data.exact.files} files · ${formatBytes(data.exact.bytes)}` : data?.r2Configured ? "Refresh to check stored files" : "File storage is not connected"}</p></div>
              <Button variant="outline" size="sm" className="h-7 shrink-0 px-2.5 text-[11px]" onClick={() => refetch()} disabled={isFetching}><RefreshCw className="mr-1 h-3 w-3" />Refresh</Button>
            </div>
            <div className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="min-w-0"><p className="text-xs font-semibold">Files needing review</p><p className="truncate text-[11px] text-muted-foreground">{untracked ? (untracked.count ? `${untracked.count} file${untracked.count === 1 ? "" : "s"} need review · ${new Date(untracked.scannedAt).toLocaleString()}` : `No files need review · ${new Date(untracked.scannedAt).toLocaleString()}`) : "Private files not linked to a saved document"}</p></div>
