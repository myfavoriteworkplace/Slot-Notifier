...
export default function PatientDocumentsTab({ bookingId, patientName, doctorName, visitDate, authorRole }: Props) {
  ...
  const [category, setCategory] = useState("X-ray"); const [description, setDescription] = useState("");
  ...
  const upload = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/patient-documents", bookingId] }); setDescription(""); if (fileRef.current) fileRef.current.value = ""; notify.success("Document uploaded"); },
  ...
  return <div className="p-3.5 space-y-3">
    <div className="rounded-md border border-slate-300 bg-slate-50/70 dark:border-slate-600 dark:bg-slate-900/20 p-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[150px_1fr_auto] gap-2 items-center">
        ...
        <input ref={fileRef} type="file" accept="image/*,.pdf" className="text-xs max-w-full" onChange={e => e.target.files?.[0] && upload.mutate(e.target.files[0])} disabled={upload.isPending} />
      </div>
    </div>
