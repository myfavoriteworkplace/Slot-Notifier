...
**SA-05 through SA-08 are now complete:** the Super Admin surfaces explicitly distinguish loading, unavailable, failed, refreshing, and delayed data for clinic directories, messaging usage, storage usage, entitlement history, and provider history. Real measured zero values remain visible as zero. The former Overview tab is now the Operations workspace with a single header, KPI area, attention area, tenant operations area, reporting-period control, and full operations refresh action. Messaging trend, channel mix, delivery outcomes, and clinic comparison now use the Operations workspace’s shared clinic detail surface without repeating the full Messaging Usage KPI row or opening a second clinic sheet. The Messaging Usage tab remains temporarily available for staged parity review, but its clinic rows open the same Operations detail surface. **SA-09 through SA-12 are now complete:** the shared Clinics & Access directory contract defines the searchable fields, lifecycle/access filters, workspace selection behavior, archived handling, directory columns, and mobile-priority fields, with focused tests; the Super Admin page now exposes a responsive Clinics & Access workspace with the shared directory on the left and selected-clinic access detail on the right; the selected-clinic detail includes Book/About URL copying, clinic editing, credential management, and archive/restore actions using the existing Admin-owned dialogs, mutations, and feedback; and the selected-clinic view now displays server-derived access guidance, recovery-Trial context, effective plan/source, subscription state, dates, limits, features, grants, exceptions, and unknown-plan handling. The next independent implementation step is **SA-13: Combine clinic history**.
...
| SA-08 | **Completed:** Create one Operations clinic detail surface | Replace separate Operations and Messaging Usage clinic sheets with one selected-clinic context containing operational, messaging, storage, and provider information. | `AdminOperationsOverview.tsx`, `AdminMessagingUsagePanel.tsx`, `Admin.tsx` | SA-04, SA-07 | A clinic opened from an attention card, tenant table, embedded messaging comparison, or legacy Messaging Usage comparison uses the same controlled detail sheet; the legacy tab remains available without owning a duplicate clinic sheet, list state is preserved, and leaving Operations closes the portaled detail surface. |
| SA-09 | **Completed:** Define the Clinics & Access directory contract | Specify the directory columns, search fields, selection model, URL/state behavior, filters, archived handling, and mobile representation. | `shared/admin-clinic-directory.ts`, `shared/admin-clinic-directory.test.ts`, `shared/admin-operations.ts` | SA-01, SA-03 | One shared contract defines the directory search fields, all lifecycle/access filters, archived precedence, workspace selection behavior, directory columns, and mobile-priority fields; focused tests cover search, selection conventions, and filter behavior without changing the visible navigation. |
| SA-10 | **Completed:** Build the Clinics & Access shell | Combine the directory and selected-clinic detail layout. Use a split layout on large screens and stacked layout on narrow screens. | `client/src/pages/Admin.tsx`, `client/src/components/AdminEntitlementReview.tsx`, `shared/admin-clinic-directory.ts` | SA-09 | The Admin page exposes a Clinics & Access workspace with shared directory search/filter behavior, archived and pending visibility, split desktop layout, stacked narrow layout, and selected-clinic access detail without switching between Active Clinics and Entitlements tabs. |
| SA-11 | **Completed:** Move clinic profile actions | Preserve edit clinic, credentials, booking/About URLs, archive, and restore actions in the shared detail view. | `client/src/pages/Admin.tsx`, `client/src/components/AdminEntitlementReview.tsx` | SA-10 | The selected-clinic detail exposes Book/About URL copying, Edit clinic, Credentials, and Archive/Restore actions; all actions reuse the existing Admin-owned dialogs, server mutations, cache invalidation, notifications, and existing Active Clinics controls. |
| SA-12 | **Completed:** Move effective access and entitlement details | Show the server-derived plan, access source, subscription state, important dates, limits, features, grants, exceptions, and unknown states. | `AdminEntitlementReview.tsx`, `shared/effective-entitlement.ts`, entitlement tests | SA-10; existing entitlement endpoints remain authoritative | The selected clinic view explains why access is active, Trial, paid, sponsored, exceptional, expired, pending, or unavailable, includes server-derived next-step guidance and recovery-Trial context, and treats unknown plans as unknown access rather than healthy paid access. |
| SA-13 | **Completed:** Combine clinic history | Present lifecycle events, plan assignments, sponsored grants, entitlement exceptions, revocations, and provider events in one consistent history view. | `AdminEntitlementReview.tsx`, `server/routes.ts`, admin subscription history contract | SA-12 | The selected clinic has one chronological access history with typed lifecycle, plan, sponsored, exception, and provider records; active sponsored access and exceptions retain their revoke actions, revoked records show their revocation timestamp, and loading, unavailable, delayed, and empty states remain explicit. |
