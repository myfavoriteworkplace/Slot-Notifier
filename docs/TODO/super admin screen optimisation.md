...
**SA-05, SA-06, and SA-07 are now complete:** the Super Admin surfaces explicitly distinguish loading, unavailable, failed, refreshing, and delayed data for clinic directories, messaging usage, storage usage, entitlement history, and provider history. Real measured zero values remain visible as zero. The former Overview tab is now the Operations workspace with a single header, KPI area, attention area, tenant operations area, reporting-period control, and full operations refresh action. Messaging trend, channel mix, and delivery outcomes now appear in Operations without repeating the full Messaging Usage KPI row. The Messaging Usage tab remains temporarily available for staged parity review and clinic-level detail. The shared clinic directory and Active Clinics/Entitlements consolidation have not started. The next independent implementation step is **SA-08: Create one Operations clinic detail surface**.
