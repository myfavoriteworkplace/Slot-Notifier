...
## Super Admin screen in plain language

The Super Admin screen is the platform control room for BookMySlot. It helps the platform team see which clinics are active, whether their subscriptions are working, how much messaging and storage they are using, and which clinics or services need attention. An administrator can search and filter clinics, open a clinic summary, review service usage, and identify problems such as failed messages, inactive subscriptions, or storage nearing its limit.

The screen is designed to help the platform team keep clinics able to use BookMySlot. It is not a clinic accounting screen: it does not show treatment revenue, patient bills, doctor earnings, clinical records, or routine patient payment history.

## Plain-language delivery status

The table below is the working summary of what the Super Admin area can do today, what has been started, and what remains to be built. “Achieved” describes the user benefit already available. “Next step” explains the remaining work in everyday language.

| Area / task | Status | What this step achieves in everyday language | What is available or achieved today | Next step / pending work | Planned release |
|---|---|---|---|---|---|
| Super Admin access and privacy boundary | **Complete** | Ensures only authorized platform administrators can use the screen and keeps clinic-private business information out of it. | The existing admin login protection remains in place. The operations view excludes treatment revenue, patient bills, doctor earnings, clinical records, and routine patient payment history. | Continue authorization and data-leakage checks as new features are added. | Release A |
| Operations overview | **Complete** | Gives the platform team one place to quickly understand whether clinics and platform services need attention. | The Overview tab is the default screen and includes tenant, subscription, messaging, storage, and attention summaries. | Add richer platform-health and incident history once the supporting data is available. | Release A |
...
| Storage warning thresholds | **Pending** | Gives early warning before a clinic runs out of space, reducing service disruption. | Storage attention cards can identify high usage, but the full warning policy is not yet complete. | Define warning levels, administrator actions, measurement freshness rules, and exact-scan status. | Release B |
| Feature provisioning | **Pending** | Lets the platform team control which platform features each clinic can use in a consistent way. | Some existing clinic capabilities and settings exist, but there is no dedicated, auditable feature-provisioning system. | Add feature access records, server-side enforcement, clear effective-state display, and audit entries for changes. | Release B |
| Platform health | **Partially available** | Helps the platform team know whether BookMySlot and its connected services are working normally. | Basic application and database health checks exist. Historical provider and webhook performance is not yet shown in the Super Admin screen. | Add historical checks, provider response health, webhook health, incidents, and alerts. | Release C |
| Tenant health score | **Pending** | Provides a simple, explainable signal for which clinics may need proactive help. | No health score is currently included. | Define transparent inputs such as subscription issues, failed messages, storage pressure, and service outages; show the reasons behind every score. | Release C |
| Unified audit viewer | **Pending** | Gives administrators one place to review important access, privacy, billing, and operational changes. | Login, PII, and billing audit sources exist separately; the shared viewer is deferred. | Create one searchable audit view with actor, clinic, action, time, reason, and safe detail. | Release B |
| Read-only support sessions | **Pending** | Allows support staff to safely understand a clinic’s issue without changing clinic data or acting as the clinic user. | No dedicated support-session flow exists yet. | Add time-limited, read-only sessions with approval, visible banners, restricted permissions, and audit logging. | Release C |
| Existing administrative areas | **Complete / retained** | Keeps established admin work available while the new operations view is introduced. | Active, Messaging Usage, Pending, Archived, Smile Deals, and Login Activity remain available. | Reorganize navigation only after the operational areas and access rules are ready. | Release A |
| Multi-branch organizations | **Deferred** | Would allow one organization to manage several clinic branches under shared rules. | The current model is clinic-based and does not support organization/branch relationships. | Design the organization and branch model, shared usage rules, and organization-level administrators. | Release D |
| Accessibility, security, migration, and release checks | **Ongoing** | Makes the screen usable, safe, and dependable as more operational controls are introduced. | Type checks, build checks, route protection, and privacy-boundary verification are part of the current release checks. | Complete responsive, accessibility, authorization, migration, data-leakage, and release verification for each future release. | Releases A–D |
