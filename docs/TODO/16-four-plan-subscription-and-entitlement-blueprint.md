...
| Subscription lifecycle | **7. Build the Trial and recovery state machine** | Implement initial Trial, seven-day read-only grace, confirmed paid-expiry recovery Trial, conversion, expiry, origin, previous-plan preservation, timezone boundaries, and provider-expiry idempotency. | Step 6 | **Complete for transition processing; notices remain.** Initial Trial creation, Trial-to-paid provider-pending conversion, provider activation, post-grace expiry reconciliation, and Razorpay completed/expired recovery use append-only history and transition/provider-event identifiers. Clinic-facing notices remain. | Boundary and provider-race tests pass; data is preserved and no endless free loop is possible. |
...
| Super Admin operations | **10. Add dedicated plan policy and assignment operations** | Add policy configuration, Start Trial, Extend Trial, Assign Paid Plan after expiry or during approved Trial conversion, Sponsored Access, mandatory reasons, confirmation, role checks, one-active-grant rules, stale-state protection, audit events, and provider-aware activation. Remove unrestricted plan mutation from generic clinic editing. | Steps 6–9 | **Partial, hardened for the next rollout slice.** Dedicated audited Trial operations, provider-aware paid-plan assignment/conversion, sponsored-access and entitlement-exception grant/revocation routes, Admin filters, lifecycle history, and the separate versioned Plan Policies registry are implemented. Generic clinic editing now rejects subscription and entitlement fields, and sponsored access has a distinct effective state and rejects overlap with active paid access. Provider reconciliation, extension/replacement workflows, delegated roles, sponsored-expiry events, and live policy cutover remain outstanding; the broad Mark Paid path is disabled. | The generic clinic update route has an explicit profile-field allowlist; the resolver and Admin UI distinguish sponsored access from paid access; grant creation checks active sponsored-period and active-paid overlap inside its transaction. |
| Provider and finance | **11. Align provider lifecycle and subscription finance** | Complete Trial conversion, recovery-to-paid assignment, upgrades, downgrades, payment grace, webhook reconciliation/idempotency, policy references, and the restricted captured/refunded/chargeback/settlement/manual-payment ledger. | Steps 6–10 | **Partial foundation, with conversion and expiry processing added.** Razorpay mapping, provider activation, Trial/recovery conversion history, duplicate-event retry handling, paid-expiry recovery, and provider-event history exist; upgrades/downgrades, full reconciliation, and the financial ledger do not. | Provider and database states reconcile; complimentary access is separate from captured cash and clinic treatment revenue. |
...
This slice intentionally does not implement warning mode, server-side enforcement, or clinic-facing Trial notices. Automatic initial Trial, paid-expiry recovery, provider conversion, and post-grace expiry processing are recorded in Sections 22.12–22.13; the production baseline remains a release gate before enabling lifecycle automation against live clinics.
...
At the time of this historical stage, the following blueprint boundaries remained intentionally deferred. Later lifecycle work is recorded in Section 22.13:

- policy draft/review/publish persistence;
- sponsored-access and exception revocation;
- provider webhook reconciliation beyond activation/recovery and the subscription finance ledger;
- clinic-facing warnings and upgrade guidance;
- server-side entitlement enforcement;
- provider-aware upgrades and downgrades.
...
The remaining lifecycle work is clinic-facing Trial/recovery notices, full provider reconciliation, and the production baseline gate before live rollout.

### 22.13 Trial conversion, post-grace expiry, and legacy Mark Paid reconciliation evidence

The next lifecycle safety slice was implemented on **2026-09-12 (Asia/Calcutta)**:

- `shared/trial-lifecycle.ts` now exposes explicit Trial phases (`active`, `grace`, `expired`, `unknown`, and `not_trial`) plus conversion and post-grace expiry predicates. Exact boundary tests cover Trial expiry, the seven-day grace boundary, missing dates, and non-Trial plans.
- The dedicated paid-plan route accepts an active or grace-period Trial for approved provider-aware conversion. It records the assignment as `converted` in `pending_payment` state, clears the Trial snapshot while payment is pending, and does not grant paid access before provider confirmation.
- `subscription.charged` and `subscription.activated` now reconcile the current clinic row inside a transaction, set paid access active only for a known paid plan, record `converted` or `renewed` lifecycle history, preserve the provider end date, consume the activation token, and clear stale Trial dates.
- Provider events left in `received` state can be retried safely. Already `applied`, `ignored`, or `unmatched` events remain idempotent and are not applied twice.
- `POST /api/internal/subscription-lifecycle/reconcile` provides a scheduler-only sweep for Trial rows past `trialGraceEndsAt`. It changes only `trialing` Trial rows, records an append-only `expired` event and assignment, and uses a clinic/grace-boundary transition ID so repeated sweeps cannot repeat the expiry.
- The Admin and clinic entitlement reads reconcile the selected clinic’s expired Trial before returning its report, so an observed post-grace clinic cannot remain indefinitely in the active Trial state while waiting for the scheduled sweep.
- The old `PATCH /api/clinics/:id/mark-paid` mutation now returns `410 Gone` without changing subscription state, and its Admin button was removed. Paid access must use the dedicated provider-aware assignment and activation path.
...
The Super Admin entitlement review now supports audited revocation of active temporary access:

- Sponsored access can be revoked through `POST /api/admin/clinics/:id/sponsored-access/:grantId/revoke`.
- Entitlement exceptions can be revoked through `POST /api/admin/clinics/:id/entitlement-exceptions/:exceptionId/revoke`.
- Both routes are Super Admin-only, require a reason of at least 10 characters, and accept an idempotency-safe transition ID.
- Revocation updates the access record with `revokedAt` and appends a `sponsored_access_revoked` or `exception_revoked` lifecycle event.
- Repeating a transition or revoking an already-revoked record is harmless and returns the existing state without creating a duplicate event.
- Expired records are not presented as active revocation targets and cannot be revoked as if they still grant access.
- The Admin screen now lists sponsored-access and exception records with Active, Ended, or Revoked status and provides a reason dialog for active-record revocation.
