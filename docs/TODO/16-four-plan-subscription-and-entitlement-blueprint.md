...
**Status:** Commercial policy approved for implementation planning; sponsored-access and platform-revenue gaps resolved; access enforcement and subscription lifecycle behavior remain unchanged; reporting-only entitlement snapshots are now available
...
| **8. Implement Trial creation, conversion, expiry, and recovery** | Give new clinics a safe trial and give expired paid clinics a short, controlled chance to recover. | Start Trial once, calculate expiry/grace dates, warn before expiry, move confirmed paid expiry to recovery Trial, preserve old plan/provider history, and make the transition idempotent. | **Partial foundation.** The explicit `trialing` state and policy-driven Trial dates now exist, and the audited manual Start/Extend Trial operation is implemented. Automatic signup, paid-expiry recovery, conversion, and expiry automation remain outstanding. | Repeated provider events cannot restart a Trial, and expiry never deletes data or silently leaves paid access active. |
| **9. Add safe Super Admin plan operations** | Let authorized staff manage plans safely without editing clinic rows directly. | Add Start Trial, Extend Trial, Assign Paid Plan after expiry, Grant Sponsored Access, reasons, confirmation, role checks, audit events, stale-state protection, and provider-aware activation. | **First vertical slice complete.** Super Admin can Start or Extend Trial through a dedicated role-protected action with required reason, policy-versioned assignment history, append-only lifecycle event, idempotency key, and stale-state safeguards. Paid-plan assignment and sponsored-access workflows remain outstanding. | Every manual plan or access change is authorized, confirmed, explainable, audited, time-bounded where applicable, and safe against provider/admin races. |
...
| Subscription lifecycle | **6. Add lifecycle and history data** | Add Trial dates, Trial origin, previous paid plan, paid-expiry time, conversion/assignment history, policy references, transition identity, sponsored-access grants, and time-limited exceptions while keeping legacy `unpaid` readable. | Steps 1–5 | **Not started.** The clinic model still mainly contains plan, billing cycle, subscription status, and Razorpay subscription ID. | Schema, idempotent migration, storage contract, audit model, and Render SQL exist; historical transitions are explainable. |
| Subscription lifecycle | **7. Build the Trial and recovery state machine** | Implement initial Trial, seven-day read-only grace, confirmed paid-expiry recovery Trial, conversion, expiry, origin, previous-plan preservation, timezone boundaries, and provider-expiry idempotency. | Step 6 | **Not started.** Catalog values exist, but Trial records, transitions, expiry processing, and recovery flow do not. | Boundary and provider-race tests pass; data is preserved and no endless free loop is possible. |
...
This slice intentionally does not implement automatic initial-Trial creation, paid-expiry recovery, paid-plan assignment, sponsored access, warning mode, or server-side enforcement. The next lifecycle slice is automatic Trial/recovery transition handling; the production baseline remains a release gate before enabling it against live clinics.