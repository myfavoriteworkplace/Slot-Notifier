...
| **Split-deployment WebSocket URL (fixed May 2026)** | When the frontend is a Render Static Site and the backend is a separate Render Web Service, `window.location.host` points to the static site — which has no WebSocket server. This caused the socket to fail instantly and silently reconnect in a loop forever. The fix: derive the WebSocket URL from `API_BASE_URL` (which maps to `VITE_API_URL`) using `.replace(/^https/, "wss")`. Falls back to `window.location.host` only when `API_BASE_URL` is empty (development / single-service deploy). If you ever change the deployment topology, verify this URL construction. |
