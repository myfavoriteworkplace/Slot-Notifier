...
## Section 0 — Deployment Environment & Schema Management

> **Read this first.** The schema management story is non-standard. Understanding it prevents broken production deployments.

### Where the app runs

| Environment | Purpose | How it starts |
|---|---|---|
| **Replit** | AI-assisted development **only** — code is written and previewed here | `npm run dev` — Express + Vite on port 5000, single process |
| **Local machine** | Developer testing before pushing to Render | Vite on `localhost:5173` + Express on a separate port — two processes |
| **Render (production)** | Live production deployment — **this is the real app** | Frontend = Render Static Site (`dist/public`); Backend = Render Web Service |

Replit is never used for production traffic. All schema changes must be verified against the **Render PostgreSQL** instance.

---

### Database on Render

The production database is a **Render-managed PostgreSQL** instance connected to the backend Web Service via the `DATABASE_URL` environment variable. The frontend Static Site has no direct database access.
...
`npm run db:push` runs **Drizzle's push command** (`drizzle-kit push`) which compares `shared/schema.ts` against the live database and applies any missing tables or columns. This runs automatically on every Render deploy.

---

### Two-layer schema management — how it works

There are **two separate mechanisms** that keep the database schema in sync. A developer must understand both.

#### Layer 1 — Drizzle ORM (`shared/schema.ts`)
This is the **primary source of truth** for all table and column definitions. Drizzle-kit reads this file and generates the SQL to apply.

- Applied on Render via `npm run db:push` during the build step.
- Used by the ORM for all type-safe queries throughout the app.
- New tables and columns should be added here first.

#### Layer 2 — Manual startup SQL (`server/index.ts`)
`server/index.ts` contains approximately 20 `CREATE TABLE IF NOT EXISTS` and `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` blocks that run **every time the server starts**, on all environments including Render.

These exist for two reasons:
1. **Columns added after initial deployment** that could not wait for a coordinated Drizzle push (e.g. `visit_completion_note`, `assigned_doctor_email`, `consent_token`).
2. **Tables that connect-pg-simple or other middleware expect** to exist before the ORM initialises (e.g. the `session` table).

Because these blocks use `IF NOT EXISTS`, they are safe to re-run on every start — they are no-ops if the column/table already exists.

#### Which layer applies first on Render?

```
Render deploy triggers:
  1. Build step  →  npm run db:push   (Drizzle schema applied)
  2. Start step  →  npm run start     (server/index.ts startup SQL runs)
```

Both run on every deploy. The startup SQL in `server/index.ts` is therefore the **safety net** for anything that might not be covered by the Drizzle push.

---

### How to add a new table or column (the correct procedure)

**Step 1 — Add to `shared/schema.ts`**
Define the new table or column in Drizzle syntax. This is the canonical definition.

**Step 2 — Add a safety block to `server/index.ts`**
Add a `CREATE TABLE IF NOT EXISTS` or `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` block in the schema-sync section of `server/index.ts`. This ensures the column exists even if `db:push` is skipped or fails.

```sql
-- Example pattern used throughout the codebase:
IF NOT EXISTS (
  SELECT 1 FROM information_schema.columns
  WHERE table_name='bookings' AND column_name='new_column'
) THEN
  ALTER TABLE bookings ADD COLUMN new_column text;
END IF;
```

**Step 3 — Run on Render production manually if urgent**
If you need the column available immediately without waiting for the next deploy, connect to the Render PostgreSQL shell and run the SQL directly:
```sql
ALTER TABLE table_name ADD COLUMN IF NOT EXISTS new_column type;
```

**Step 4 — Document the SQL here**
Any new column or table added should be listed in the relevant table section of this document.

---

### Node version

Pinned to **Node 20.20.0** via `.node-version` in the repo root. Render, nvm, and Volta all respect this file. Do not change it — Node 22+ causes `npm install` to fail on Render with "Exit handler never called" due to an incompatibility with the `package-lock.json` generated by npm 10.

---

### `package-lock.json` hygiene

Replit routes npm downloads through an internal proxy (`package-firewall.replit.local`) and writes that hostname into `package-lock.json`. That hostname is unreachable outside Replit — Render's build will fail with `ENOTFOUND`.

After installing any package inside Replit, always run:
```bash
npm run fix-lockfile
```
This replaces the internal Replit URLs with `https://registry.npmjs.org/`. It is a no-op if no Replit URLs are present, so it is always safe to run.
